...
must_haves:
  truths:
    ...
    - "Redis cache is set for each analytics endpoint (mock asserts cache_set is called in test_analytics_router.py)"
  artifacts:
    ...
    - path: "stock-prediction-platform/services/api/app/services/flink_service.py"
      ...
      exports: ["get_flink_jobs", "get_analytics_summary"]
...
<task type="auto">
  <name>Task 1: Pydantic schemas + config env vars + service stubs + unit tests</name>
  <files>
    stock-prediction-platform/services/api/app/models/schemas.py
    stock-prediction-platform/services/api/app/config.py
    stock-prediction-platform/services/api/app/services/flink_service.py
...
**Step 6 — Write test files** (tests are written alongside implementation in the same task commit).
...
Create `tests/test_analytics_argocd.py`:
```python
...
```
  ...
  <verify>
    <automated>cd /home/tempa/Desktop/priv-project/stock-prediction-platform/services/api && python -m pytest tests/test_analytics_flink.py tests/test_analytics_feast.py tests/test_analytics_kafka.py tests/test_analytics_argocd.py -x -q 2>&1 | tail -20</automated>
  </verify>
  <done>All 10 unit tests pass (2 flink + 2 feast + 2 kafka + 4 argocd). Seven new files created: five service/schema modules + four test files. No existing tests broken.</done>
</task>
...
```
  ...
  <verify>
    <automated>cd /home/tempa/Desktop/priv-project/stock-prediction-platform/services/api && python -m pytest tests/test_analytics_router.py tests/test_analytics_flink.py tests/test_analytics_feast.py tests/test_analytics_kafka.py tests/test_analytics_argocd.py -x -q 2>&1 | tail -20</automated>
  </verify>
...
1. `cd stock-prediction-platform/services/api && python -m pytest tests/test_analytics_flink.py tests/test_analytics_feast.py tests/test_analytics_kafka.py tests/test_analytics_argocd.py tests/test_analytics_router.py -v` — all pass
...
- 14 unit + integration tests pass (10 unit + 4 integration); no existing tests regress
</success_criteria>
